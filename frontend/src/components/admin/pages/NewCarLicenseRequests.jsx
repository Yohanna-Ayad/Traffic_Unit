...
export function LicenseRequests() {
    ...
    useEffect(() => {
        // Fetch license requests from the backend
        axios.get('http://localhost:8626/admin/getAllNewVehicleRequests', {
    ...
    const handleApprove = (requestId) => {
        axios.post(`http://localhost:8626/admin/approveLicense/${requestId}`, {}, {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        })
            .then(response => {
                toast.success('License request approved successfully');
                setRequests(requests.filter(request => request.requestId !== requestId));

            })
            .catch(error => {
                console.error('Error approving license request:', error);
                toast.error('Failed to approve license request');
            });
    };
    const handleDecline = (requestId) => {
        axios.post(`http://localhost:8626/admin/declineLicense/${requestId}`, {}, {
            headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
        })
            .then(response => {
                toast.success('License request declined successfully');
                setRequests(requests.filter(request => request.requestId !== requestId));
            })
            .catch(error => {
                console.error('Error declining license request:', error);
                toast.error('Failed to decline license request');
            });
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-800">New Car License Requests</h1>
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {requests.map((request) => (
                            <tr key={request.requestId} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4">{request.licenseType}</td>
                    ...
                    <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        <div className="p-6">
                            <div className="flex justify-between items-center mb-4">
                                <h2 className="text-xl font-bold text-gray-800">License Request Details</h2>
                                <button
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="mt-1">
                                            ...
                                            <p className="text-sm text-gray-900">
                                                <span className="font-medium">Email:</span> {selectedRequest.userEmail}
                                            </p>
                                        </div>
                                    </div>

                                    <div>
                                        <h3 className="text-sm font-medium text-gray-500">License Details</h3>
                                        <div className="mt-1">
                                            <p className="text-sm text-gray-900">
                                            ...
                                            {/* <p className="text-sm text-gray-900">
                        <span className="font-medium">Vehicle Type:</span> {selectedRequest.vehicleType}
                      </p> */}
                                            <p className="text-sm text-gray-900">
                                            ...
                                            <p className="text-sm text-gray-900">
                                                <span className="font-medium">Request Date:</span> {new Date(selectedRequest.requestDate).toLocaleString()}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <div className="mt-1">
                                            <p className="text-sm text-gray-900">
                                                <span className="font-medium">Make:</span> {selectedRequest.Vehicle.maker}
                                            </p>
                                            <p className="text-sm text-gray-900">
                                                <span className="font-medium">Model:</span> {selectedRequest.vehicleModel}
                                            </p>
                                            <p className="text-sm text-gray-900">
                                                <span className="font-medium">Year:</span> {selectedRequest.vehicleYear}
                                            </p>
                                            {/* <p className="text-sm text-gray-900">
                        <span className="font-medium">Plate Number:</span> {selectedRequest.plateNumber}
                      </p> */}
                                        </div>
                                    </div>
                        ...
                        <div className="p-4 text-center text-gray-500">
                          No documents available
                            ...
                            <div className="mt-6 flex justify-end space-x-4">
                                <button
                                    onClick={() => handleDecline(selectedRequest.requestId)}
