  ...
  // Function to create a new user          Done
    createUser:  async (req, res) => {
        try {
          ...
          if (user === 'All fields are required!' ||  user === 'Password must be at least 8 characters!') {
      ...
      resetPassword: async (req, res) => {
        try {
          const user = await userServices.resetPassword(req.body.email, req.body.code, req.body.password);
